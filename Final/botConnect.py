...
async def myhelp(ctx):
    help_text = (
        ...
        "--> `drive` Puedes descargar archivos en el ordenador desde drive\n"
...
# Convierte los archivos de un USB en un zip y los guarda en "Videos"
@bot.command()
async def usbzip(ctx, drive: str):
    import os
    import zipfile
    from datetime import datetime

    # Asegura que la letra termina en ":" (por ejemplo, E:)
    if not drive.endswith(':'):
        drive += ':'
    
    # Verifica si la ruta existe
    if not os.path.exists(drive + '\\'):
        await ctx.send(f"```Error: La unidad {drive} no existe.```")
        return

    # Obtiene la carpeta Videos del usuario actual
    videos = os.path.join(os.path.expanduser('~'), 'Videos')
    date = datetime.now().strftime('%Y%m%d_%H%M%S')
    zip_name = f"USB_Backup_{drive[0]}_{date}.zip"
    zip_path = os.path.join(videos, zip_name)
    ...
    try:
        ...
        await ctx.send(f"✅ Backup completado: `{zip_path}`")
    except Exception as e:
        await ctx.send(f"```Error al crear el ZIP:\n{str(e)}```")
...
@bot.command()
async def drive(ctx, link: str, *, path: str):
    try:
        os.makedirs(os.path.dirname(path), exist_ok=True)
        gdown.download(url = link, output=path, quiet=True, fuzzy=True)

        await ctx.send(f"```Archivo descargado en: {path}```")
    except Exception as e:
        await ctx.send(f"```Error al descargar el archivo:\n{str(e)}```")
